...
const POLL_INTERVAL = Number(process.env.POLL_INTERVAL);
  ...
  {
    const io = getIO();
    io.emit("poller-started",{timestamp: Date.now(),poller:"Poller_1_top"});
    ...
    while (true) 
    {
      ...
      try 
      {
            ...
            {
                io.emit("locking-and-processing",{timestamp: Date.now(),poller:"Poller_1_top",data:{id:event.id,payload:JSON.stringify(event.payload, null, 2)}});
  ...
  async function handleEvent(event) 
  {
    const io = getIO();
    console.log("\n");
    io.emit("processing",{timestamp: Date.now(),poller:"Poller_1_top",data:{id:event.id}})
    ...
    {
        io.emit("kafka-failure", {timestamp: Date.now(),poller:"Poller_1_top",data:{id:event.id}});
    ...
    try 
    {
      ...
      io.emit("kafka-success", {timestamp: Date.now(),poller:"Poller_1_top",data:{id:event.id}});
